...
# Build lines changed display
lines_info=""
if [ "$lines_added" -gt 0 ] || [ "$lines_removed" -gt 0 ]; then
    lines_info=" ${GRAY}|${NC} ${GREEN}+${lines_added}${NC} ${RED}-${lines_removed}${NC}"
fi

# Output the status line
echo -e "${BLUE}${dir_name}${NC} ${GRAY}|${NC} ${CYAN}${model_name}${NC} ${GRAY}|${NC} ${context_info}${lines_info}${git_info:+ ${GRAY}|${NC}}${git_info}${cost_info}"
